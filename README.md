...
The Status page is a direct example of this: logs from an automation pipeline running on my home server are aggregated every five minutes and pushed to Cloudflare KV. The Astro SSR worker fetches that payload on each request and renders four SVG charts client-side — no charting library involved. The goal is to show what an SSR site can do, rather than describe it.
...
hetサーバーで5分ごとにKVへ書き込み、AstroのSSRエンドポイントがそれをJSONで返す3層構造がStatusページのバックボーンです。静的サイトでは実現できない、データと画面が常に連動する構成をCloudflare Pagesの上で動かしています。

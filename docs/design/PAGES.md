...
1. Hero     tag label / h1（display） / 説明文（body） / stack badges
2. Demo     インタラクティブコンテンツ
3. Architecture  テキスト図 + パネルグリッド（panel コンポーネント）
4. Links    制作記事 / GitHub

### 例外: live-demo（/live-demo/）

SSR 構成のため Hero セクションなし。
StatusPage.astro が Demo〜Links を担当。
Hero を追加する場合は StatusPage.astro の改修が必要。
